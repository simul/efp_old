...
ElasticFrameMessages ElasticFrameProtocol::unpackType1(const uint8_t* pSubPacket, size_t packetSize, uint8_t fromSource) {
	...
	uint8_t hFrameType = lType1Frame.hFrameType;
	uint8_t  hStream = lType1Frame.hStream;
	uint16_t hSuperFrameNo = lType1Frame.hSuperFrameNo;
	uint16_t hFragmentNo = lType1Frame.hOfFragmentNo;
	uint16_t hOfFragmentNo = lType1Frame.hOfFragmentNo;
...
ElasticFrameMessages ElasticFrameProtocol::unpackType2(const uint8_t* pSubPacket, size_t packetSize,
	...
	uint8_t hFrameType = lType2Frame.hFrameType;
	uint8_t  hStream = lType2Frame.hStream;
	ElasticFrameContent hDataContent = lType2Frame.hDataContent;
	uint16_t hSizeOfData = lType2Frame.hSizeOfData;
	uint16_t hSuperFrameNo = lType2Frame.hSuperFrameNo;
	uint16_t hOfFragmentNo = lType2Frame.hOfFragmentNo;
	uint16_t hType1PacketSize = lType2Frame.hType1PacketSize;
	uint64_t hPts = lType2Frame.hPts;
	uint32_t hDtsPtsDiff = lType2Frame.hDtsPtsDiff;
	uint32_t hCode = lType2Frame.hCode;
...
ElasticFrameProtocol::packAndSendFromPtr(const uint8_t* rPacket, size_t packetSize, ElasticFrameContent dataContent, uint64_t pts, uint64_t dts,
	...
	if (lDataLeftToSend + sizeof(ElasticFrameType2) > mCurrentMTU) {
		LOGGER(true, LOGG_FATAL, "Calculation bug.. Value that made me sink ->" << packetSize)
